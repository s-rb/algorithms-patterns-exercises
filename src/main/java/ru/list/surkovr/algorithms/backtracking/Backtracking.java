...
public class Backtracking {
    ...
    public static List<List<Integer>> allPathsSourceTarget(int[][] graph) {
        doBackTrack(graph, new ArrayList<>(){{add(0);}}, 0);
        return res;
    }

    private static void doBackTrack(int[][] graph, List<Integer> path, int lastIndex) {
        ...
        int[] subset = graph[lastIndex];
        for (int i = 0; i < subset.length; i++) {
            path.add(subset[i]);
            doBackTrack(graph, path, subset[i]);
